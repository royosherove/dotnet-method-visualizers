...
class Program
{
    ...
    static void M3()
    {
        // be sure to include an expression that makes use of the lambda globals
        Expression<Func<int>> func = () => Math.Max(42, "TEST".Length);
        Func<int> expr = func.Compile();
        TestShowVisualizer(expr);
    }
    ...
    static void M4()
    {
        ...
        string sValue = "TEST";
        int iValue = 42;
        Expression<Func<int>> func = () => Math.Max(iValue, sValue.Length);
        Func<int> expr = func.Compile();
        TestShowVisualizer(expr);
    }
